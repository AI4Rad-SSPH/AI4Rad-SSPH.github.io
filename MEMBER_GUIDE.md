...
### D. Rebuild the manifest · 重建索引

After adding/changing any paper, regenerate the manifest so the site picks it up:
新增或修改论文后，重建索引让站点识别：
...
`data/publications/manifest.json` sorted by year (newest first). 该脚本会校验字段、核对 author_ids、按年份降序重写 manifest。

Commit both the new paper JSON **and** the regenerated `manifest.json` together.
请把新论文 JSON **和** 重建后的 `manifest.json` 一起提交。
...
- ✅ **Run `build-pubs.py`** if you touched `data/publications/`. 改了论文就跑一次 build 脚本。
